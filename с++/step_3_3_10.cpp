...
конструктора в поле str должен оказаться указатель на C-строку c копией исходной строки, а в поле size — длина строки 
...
struct String {
	...
	/*
	По сути эту простую запись можно и раскрыть:
	size = strlen(str);
